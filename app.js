...
const CHAMPIONS_MOVE_OVERRIDES={
  ...
  "moonblast":{effectChance:10,meta:{statChance:10}},
  "iron-head":{effectChance:20,meta:{flinchChance:20}},
  "salt-cure":{championsNote:"継続ダメージ半減"},
  "snap-trap":{type:"はがね"},
  "dire-claw":{effectChance:30,meta:{ailmentChance:30}},
};
...
function getEffectiveMoveEntry(entry){if(!entry)return null;const override=CHAMPIONS_MOVE_OVERRIDES[entry.apiName];if(!override)return entry;return{...entry,...override,meta:{...(entry.meta||{}),...(override.meta||{})},flags:override.flags?[...override.flags]:[...(entry.flags||[])],statChanges:override.statChanges??entry.statChanges,aliases:entry.aliases};}
function getSelectedMove(){return getEffectiveMoveEntry(dataStore.selectedMove);}
...
function weatherBallTypeName(weather){return({sun:"ほのお",rain:"みず",sand:"いわ",snow:"こおり"})[weather]||"ノーマル";}
function resolveDynamicMoveDisplay(moveEntry){const baseType=byId("moveType").value||moveEntry?.type||"-";const selectedVariablePower=getSelectedVariablePower(moveEntry);let power=selectedVariablePower??sanitizeNumber(byId("movePower").value,0);let type=baseType;const activeWeather=getActiveWeather();if(moveEntry?.apiName==="weather-ball"&&activeWeather!=="none"){type=weatherBallTypeName(activeWeather);power=100;}return{type,power};}
...
function calculateDamage(){
  ...
  if(moveEntry?.championsNote){
    moveSpecificLabels.push(moveEntry.championsNote);
  }
  ...
  if(moveApiName==="weather-ball"&&activeWeather!=="none"){
    moveType=weatherBallTypeName(activeWeather);
    effectiveMovePower=100;
    moveSpecificLabels.push("ウェザーボール");
  }
  if((moveApiName==="solar-beam"||moveApiName==="solar-blade")&&["rain","sand","snow"].includes(activeWeather)){
...
function applyMoveEntry(entry){const previousVariablePower=byId("moveVariablePower")?.value;const effectiveEntry=getEffectiveMoveEntry(entry);dataStore.selectedMove=entry;byId("moveName").value=effectiveEntry.nameJa||effectiveEntry.nameEn||effectiveEntry.apiName;byId("moveType").value=effectiveEntry.type||"";byId("moveCategory").value=effectiveEntry.category==="special"?"special":"physical";byId("movePower").value=String(effectiveEntry.power||0);byId("moveAccuracy").value=effectiveEntry.accuracy==null?"--":String(effectiveEntry.accuracy);if(effectiveEntry.target)byId("moveTarget").value=effectiveEntry.target;updateMoveHitOptions(effectiveEntry);syncVariablePowerInput(effectiveEntry,previousVariablePower);syncMoveSpecificFields(effectiveEntry);updateCalculator();}
